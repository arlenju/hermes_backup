...
§
Horizon新闻雷达(~/My_Project/Horizon/)已跑通:cron 0dc9baae5ad2每天8:30推飞书。config provider=doubao+_NO_RESPONSE_FORMAT含doubao(绕Ark不支持response_format)+languages=[zh](只要中文)。.env用/api/plan/v3端点。Crawl4AI v0.9.0装在Hermes venv(CLI:crwl, API:AsyncWebCrawler+PruningContentFilter via DefaultMarkdownGenerator)。
§
