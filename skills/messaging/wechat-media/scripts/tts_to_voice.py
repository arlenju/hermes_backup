...
async def generate_tts(text: str, voice: str = "zh-CN-XiaoxiaoNeural", output: str = None) -> str:
    ...
    print(f"  ✅ TTS: {output} ({size/1024:.1f} KB)")
    return output


def mp3_to_silk(mp3_path: str, silk_path: str = None) -> str:
    """Convert MP3 to SILK v3 (WeChat native voice format)"""
    if silk_path is None:
        silk_path = os.path.splitext(mp3_path)[0] + ".silk"
    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp:
        wav_path = tmp.name
    try:
        subprocess.run(["ffmpeg", "-y", "-i", mp3_path, "-ar", "24000", "-ac", "1",
                        "-sample_fmt", "s16", wav_path], check=True, capture_output=True, timeout=30)
        import wave
        ...
        print(f"  ✅ SILK: {silk_path} ({silk_sz/1024:.1f} KB, {silk_sz/mp3_sz*100:.0f}% of MP3)")
        return silk_path
    finally:
...
async def main():
    ...
    if not silk_path.endswith(".silk"):
        silk_path += ".silk"
    print(f"🎤 \"{text}\"")
    ...
    with open(result, 'rb') as f:
        h = f.read(10)
    valid = h[:4] == b'\x02#!S' and b'SILK' in h[:10]
    print(f"\n✅ {'Valid SILK' if valid else '⚠️  Unknown header'} — {os.path.abspath(result)}")
    if valid:
        print(f"   Include in reply: MEDIA:{os.path.abspath(result)}")
