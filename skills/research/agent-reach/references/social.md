...
> **没装 Cookie 也能干活**：用户问「X 上最近什么火 / 热搜是什么 / 大家在讨论什么」时，**先看 [trends-without-auth.md](trends-without-auth.md)**（trends24 实时热搜 + Exa 跨站搜索），不要让用户先去配 Cookie 再回来。Cookie 路线只在需要时间线/回复树/账号搜索时才升级。
