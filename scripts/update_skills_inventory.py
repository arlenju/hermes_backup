...
def scan_skills() -> dict[str, list]:
    ...
    for entry in sorted(SKILLS_DIR.iterdir()):
        ...
        # Check if this entry is a category dir (has subdirs with SKILL.md)
        ...
        if has_sub_skills:
            ...
            for skill_dir in sorted(entry.iterdir()):
                ...
                if category not in skills:
                    skills[category] = []
                skills[category].append((name, desc))
        else:
            # This is a direct skill dir (uncategorized)
...
def get_canonical_category(cat: str, dir_name: str) -> str:
    """Map a directory name or category to our canonical ordering."""
    cat_lower = cat.lower().replace("_", "-").replace(" ", "-")
    # Check against known categories
    for canonical, _, _ in CATEGORY_ORDER:
        if cat_lower == canonical or cat_lower.replace("-", "") == canonical.replace("-", ""):
            return canonical
    return dir_name if dir_name else "uncategorized"
...
def generate_markdown(skills: dict) -> str:
    ...
    lines = [
        f"# Hermes Agent — Skills Inventory",
        "",
    ...
    for canonical, icon, display_name in CATEGORY_ORDER:
        if canonical in skills and skills[canonical]:
            lines.append(f"- [{icon} {display_name}](#{canonical})")
    ...
    lines.append("---")
    lines.append("")
    lines.append(f"> 生成工具：Hermes Agent | 脚本：`~/.hermes/scripts/update_skills_inventory.py`")
    lines.append("")
