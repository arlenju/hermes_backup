...
§
已配 Agently Mail CLI (@tencent-qqmail/agently-cli v1.0.5)，邮箱 arlen0342@agent.qq.com。OAuth 凭据存 macOS keychain。支持: 收件箱列表、搜索、发送/回复/转发（两阶段确认）、下载附件、移回收站。skill 已安装到 Hermes。命令前缀 agently-cli。
§
已配 Agently Mail (npm @tencent-qqmail/agently-cli v1.0.5, skill已装)。邮箱 arlen0342@agent.qq.com, OAuth macOS keychain。支持收发/搜索/附件/回收站(两阶段确认)。用户邮箱 jushuai_01@163.com。
§
